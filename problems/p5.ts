...
export const getAllMoviesWithAverageScoreOverN = async (n: number) => {
  const starRatingsGroupedByMovieIdWithScoreGreaterThanN =
    await prisma.starRating.groupBy({
      by: ["movieId"],
      having: {
        score: {
          _avg: {
  ...
  return prisma.movie.findMany({
    where: {
      id: {
        in: starRatingsGroupedByMovieIdWithScoreGreaterThanN.map(
          (starRating: { movieId: number }) => starRating.movieId
        ),
      },
    },
  });
